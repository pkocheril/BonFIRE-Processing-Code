...
%%% change if length() == 1 to isscalar (?)
%%% use "reshape" in reading .raw files
%%% eliminate pre-loop by guessing max length to be a large number
%%% switch to "current vs set" for values - let vary per-folder
%%% add snrv2 (using stdev of baseline)
%%% updated variable names (more descriptive)
%%% switch from hard-coded analysis to modular functions


% To Do
%%% make 6d master array for images (4D array per pixel) - takes too much RAM
%%% replace tiff writeout with raw writeout - do lt1,lt2,ratio,r2,etc.
%%% use functions for long chunks of code (file info reading, fitting, etc.)
%%% add dialog for pulling values from master array and write to a table with
% labeled columns

%%% use image preview and cursor to select ROI for image test fit

%%% change filetype assignment to be per file (by filename)
%%% add idler tuning rate (~6.5 fs/cm-1 for CH-stretching region)

%%% add CH3,CH4,SPCM simultaneous processing - let user select channels to fit

%%% v0 - has lots of old code commented out and troubleshooting/verification
...
fitchannels = []; % specify data channels to fit, [] = dialog
...
if isempty(loadprevious) == 1 % auto-detect previous analysis
    ymllist = dir(fullfile(workingdirectory,'*.yml')); % look for .yml files
    if isempty(ymllist) == 1 % no .yml files
        loadprevious = 0;
    else % found a .yml file
        if isscalar(ymllist) % found 1 .yml file
            loadprevious = 1;
...
if runtype == 2 % to examine a single file - choose one folder
    [indx,~] = listdlg('PromptString',{'Select one folder.'},...
    'SelectionMode','single','ListString',subfolders);
    targetfolders = indx;
end

allchannels = {'CH1','CH2','CH3','CH4','SPCM'};
...
if isempty(fitchannels) % dialog to select fit channels
    [indx,~] = listdlg('PromptString',...
        ...
        'multiple','ListString',allchannels);
    fitchannels = indx;
end
...
indcrawCH1 = indcffty+1; % 17
indccorrCH1 = indcrawCH1+1; % 18
indcCH1base = indccorrCH1+1; % 19
indcrawCH3 = indcCH1base+1; % 20
indccorrCH3 = indcrawCH3+1; % 21
indcCH3base = indccorrCH3+1; % 22
indcrawCH4 = indcCH3base+1; % 23
indccorrCH4 = indcrawCH4+1; % 24
indcCH4base = indccorrCH4+1; % 25
indcrawSPCM = indcCH4base+1; % 26
indccorrSPCM = indcrawSPCM+1; % 27
indcSPCMbase = indccorrSPCM+1; % 28
...
indrsnrv2 = indrsnr+1; % 8
indrsbr = indrsnr+1; % 9
indrnoise = indrsbr+1; % 10
indrbasesd = indrnoise+1; % 11
indrintsnr = indrbasesd+1; % 12
indrbleach = indrintsnr+1; % 13
indrtlist = indrbleach+1; % 14
indrlifetime1 = indrtlist+1; % 15
indrlifetime2 = indrlifetime1+1; % 16
indrlt1lt2 = indrlifetime2+1; % 17
indrnfiles = indrlt1lt2+1; % 18
indralignlength = indrnfiles+1; % 19
indrIRpower = indralignlength+1; % 20
indrprbpower = indrIRpower+1; % 21
indrpulsewidth = indrprbpower+1; % 22
indrCH1peak = indrpulsewidth+1; % 23
indrCH1peaksd = indrCH1peak+1; % 24
indrintegCH1 = indrCH1peaksd+1; % 25
indrCH1integsd = indrintegCH1+1; % 26
indrCH1snr = indrCH1integsd+1; % 27
indrCH1snrv2 = indrCH1snr+1; % 28
indrCH1sbr = indrCH1snrv2+1; % 29
indrCH1noise = indrCH1sbr+1; % 30
indrCH1bleach = indrCH1noise+1; % 31
indrCH3peak = indrCH1bleach+1; % 32
indrCH3peaksd = indrCH3peak+1; % 33
indrintegCH3 = indrCH3peaksd+1; % 34
indrCH3integsd = indrintegCH3+1; % 35
indrCH3snr = indrCH3integsd+1; % 36
indrCH3snrv2 = indrCH3snr+1; % 37
indrCH3sbr = indrCH3snrv2+1; % 38
indrCH3noise = indrCH3sbr+1; % 39
indrCH3bleach = indrCH3noise+1; % 40
indrCH4peak = indrCH3bleach+1; % 41
indrCH4peaksd = indrCH4peak+1; % 42
indrintegCH4 = indrCH4peaksd+1; % 43
indrCH4integsd = indrintegCH4+1; % 44
indrCH4snr = indrCH4integsd+1; % 45
indrCH4snrv2 = indrCH4snr+1; % 46
indrCH4sbr = indrCH4snrv2+1; % 47
indrCH4noise = indrCH4sbr+1; % 48
indrCH4bleach = indrCH4noise+1; % 49
indrSPCMpeak = indrCH4bleach+1; % 50
indrSPCMpeaksd = indrSPCMpeak+1; % 51
indrintegSPCM = indrSPCMpeaksd+1; % 52
indrSPCMintegsd = indrintegSPCM+1; % 53
indrSPCMsnr = indrSPCMintegsd+1; % 54
indrSPCMsnrv2 = indrSPCMsnr+1; % 55
indrSPCMsbr = indrSPCMsnrv2+1; % 56
indrSPCMnoise = indrSPCMsbr+1; % 57
indrSPCMbleach = indrSPCMnoise+1; % 58

if loadprevious == 0 % run new analysis
    % Make master array to store all data (row,column,folder,file,pixelx,pixely)
    maxTlength = 4000; % guess max Tlength with extra length for tD compensation
    maxfilesinfolder = 200; % guess max number of files in a folder
    master = NaN(maxTlength,indcSPCMbase,length(folders),maxfilesinfolder);
    ...
    for ii = folders % loop through folders
        ...
        currltfittype = setvalue(ltfittype, folders,ii);
        currbasefittype = setvalue(basefittype,folders,ii);
        currdatatype = setvalue(setdatatype,folders,ii);
        currpulsewidth = setvalue(setpulsewidth,folders,ii);
        currfiletype = setvalue(setfiletype,folders,ii); % setting file type manually per-folder - to be overridden by per-file autofinding if needed
        currfilenameconv = setvalue(setfilenameconv,folders,ii);

        % Look for files
        %%% look for CH2 raws, then use exist() == 2 to check for other channels
        rawlist = dir(fullfile(workingdirectory,subfolders{ii},'*.raw')); rawlist = {rawlist(~[rawlist.isdir]).name};
        % CH1raw = dir(fullfile(D,N{ii},'*CH1.raw')); CH1raw = {CH1raw(~[CH1raw.isdir]).name};
        CH2raw = dir(fullfile(workingdirectory,subfolders{ii},'*CH2.raw')); CH2raw = {CH2raw(~[CH2raw.isdir]).name};
        % CH3raw = dir(fullfile(D,N{ii},'*CH3.raw')); CH3raw = {CH3raw(~[CH3raw.isdir]).name};
        % CH4raw = dir(fullfile(D,N{ii},'*CH4.raw')); CH4raw = {CH4raw(~[CH4raw.isdir]).name};
        % SPCMraw = dir(fullfile(D,N{ii},'*SPCM.raw')); SPCMraw = {SPCMraw(~[SPCMraw.isdir]).name};
        ...
        %%% look for per-file Tlist first, then Tlist.txt, then auto-generate
        % Tlistlist = dir(fullfile(workingdirectory,subfolders{ii},'*Tlist.txt')); Tlistlist = {Tlistlist(~[Tlistlist.isdir]).name};
        % if isempty(rawlist)
        %     channels = 0; % data are txts
        % end
        % if ~isempty(CH1raw)
        %     channels(1) = 1;
        % else
        ...
        %     channels(5) = 0;
        % end
        % if length(CH2raw) == length(Tlistlist)
        %     Tlistperfile = 1;
        % else
        %     Tlistperfile = 0;
        %     if length(CH2raw) >= 2 && isscalar(Tlistlist) % only Tlist.txt
        %     else % multiple Tlists
        %         warning('Tlist file confusion - using first Tlist')
        %     end
        % end

        if numel(CH2raw) == 0 % CH2 raw files not found
            if numel(rawlist) == 0 % no .raw files
                if numel(tiflist) == 0 % no .tif files
        ...
        %%% move to per-file basis - use lookforFOV
        % % On both passes - examine files in folders
        % if isempty(setfiletype) == 1 % figure out data type on a per-folder basis
        %     autotype = 1;
        %     currfiletype = [];
        %     pairedDCAC = [];
        % else
        %     autotype = 0;
        %     currfiletype = setfiletype;
        % end
        ...
        if runtype == 2 % to examine a single file - choose file
            ...
            targetfile = indx2;
            totalfilesC = targetfile; % load only chosen file
            startjj = targetfile;
        else
            startjj = 1;
            if runtype > 0 % limit test run files per folder
                ...
                totalfilesC = numel(datafiles); % all files
            end
        end
        ...
        if ~isempty(datafiles) % skip subfolders without data files
            for jj = startjj:totalfilesC % jj = file number in subfolder ii
                ...
                if isempty(currdatatype)
                    ...
                    % lookforFOV = strfind(currentfile,'FOV');
                    % if isempty(lookforFOV) == 1 % filename doesn't have 'FOV'
                ...
                % Figure out file naming convention and file type
                if isempty(currfilenameconv)
                    if contains(currentfile,'.txt') %count(currentfile,'.txt') > 0
                        ...
                        if strcmp(filename(5:9),'cm-1_') % filename ####cm-1_...
                            currfilenameconv = 3; % 2DVF probe sweep
                        else
                ...
                %         filename = char(folderinfo(end));
                %         if strcmp(filename(5:9),'cm-1_') % filename ####cm-1_...
                %             setfilenameconv = 3; % 2DVF probe sweep
                %         else
                %             setfilenameconv = 2; % not a 2DVF probe sweep
                %         end
                %     end
                % %%% not function b/c only should happen once
                % %[currdatatype,currfilenameconv,] = parsefilename(currentfile,currdatatype,currfilenameconv);
                

%%%%
                % if isempty(currfiletype) == 1 % figure out file type
                %     checkraw = dir(fullfile(workingdirectory,subfolders{ii},'*CH2.raw')); % check for raw files
                %     checkraw = {checkraw(~[checkraw.isdir]).name};
                %     if isempty(setdatatype) == 1
                %         if isempty(checkraw)
                %             warning('No raw files found - data assumed to be a solution sample. Specify data type and re-run if needed.');
                %             setdatatype = 0;
                %         else
                %             lookforFOV = strfind(checkraw(:),'FOV');
                %             if isempty(lookforFOV) == 0 % file names don't have 'FOV'
                %                 setdatatype = 0; % solution data
                %             else % file names have 'FOV'
                %                 setdatatype = 1; % image data
                %             end
                %         end
                %     end
                % 
                ...
                %         if setdatatype == 1 % image data
                %             currfiletype = 5;
                %         else
                %             currfiletype = 4; % solution data
                %         end
                %         if isempty(pairedDCAC) == 1
                %             CH1raw = dir(fullfile(workingdirectory,subfolders{ii},'*CH1.raw')); % check for CH1 files
                %             CH1raw = {CH1raw(~[CH1raw.isdir]).name};
                %             SPCMraw = dir(fullfile(workingdirectory,subfolders{ii},'*SPCM.raw')); % check for SPCM files
                ...
                %         end
                %     else % no raw files
                %         checktif = dir(fullfile(workingdirectory,subfolders{ii},'*CH2.tif')); % check for tif files
                %         checktif = {checktif(~[checktif.isdir]).name};
                %         if numel(checktif) > 0 % tif files present
                %             if setdatatype == 1 % image data
                %                 currfiletype = 3;
                %             else
                %                 currfiletype = 2; % solution data
                %             end
                %             if isempty(pairedDCAC) == 1
                %                 CH1tif = dir(fullfile(workingdirectory,subfolders{ii},'*CH1.tif')); % check for CH1 files
                %                 CH1tif = {CH1tif(~[CH1tif.isdir]).name};
                %                 SPCMtif = dir(fullfile(workingdirectory,subfolders{ii},'*SPCM.tif')); % check for SPCM files
                ...
                %             checktxt = dir(fullfile(workingdirectory,subfolders{ii},'*.txt'));
...
%%%%
                ...
                if currfilenameconv == 1 % parse .txts
                    ...
                    imgdim = char(fileinfo(end-4)); imgdimsplit = split(string(imgdim),"X"); xsteps = imgdimsplit(1); ysteps = imgdimsplit(end); xsteps = sscanf(xsteps,'%f'); ysteps = sscanf(ysteps,'%f'); % '5X5' --> 5 and 5
                ...
                if currfilenameconv == 4 % IR sweep (deprecated)
                ...
                % end
                % prbpower = prbpower/(10^(ND)); % power loss from ND filter
                ...
                [delaypos,data,imagesize] = ...
                    ...
                    currfiletype,trimlastT,trimfirstT,xsteps,ysteps,2);

%%% if .txt, run loaddata twice to get CH1 and CH2
                %[~,ch1data,ch1signal,ch1sds,~] = loaddata(...,ch1file,...,1)

%%% should look for and find all existing channels - use isfile()

                if currdatatype == 1 % for image data - make arrays to write outputs into
                ...
                %     Tlistname = fullfile(workingdirectory,subfolders{ii},'Tlist.txt'); % current Tlist
                ...
                %             end
                %         end
                %         data = reshape(imgvector,[xsteps,ysteps,length(x)]);
                %         % for i=1:length(x) % flip x and y
                %         %     data(:,:,i) = data(:,:,i).';
                %         % end
                %     end
                %     imagesize = size(data);
                ...
                %                 DCdata = reshape(DCimgvector,[xsteps,ysteps,length(x)]);
                ...
                %         end             
                %     end
                %     if currfiletype == 2 || currfiletype == 4 % avg & stdev of XY data (|| = OR)
                ...
                %%% replace with image pop-up and select ROI with cursor
                if runtype > 0 && currdatatype == 1 % do test run in the middle of an image
                    preview = image(data(:,:,1),'CDataMapping','Scaled'); colormap('hot'); colorbar; datacursormode;
                    prompt = {'Specify X coordinate.','Specify Y coordinate.'};
                    dlgtitle = 'Choose test run pixel';
                    dims = [1 45; 1 45];
                    definput = {'0','0'};
                ...
                if currdatatype == 0 % solution data
                ...
                for iii=startiii:imagesize(1)
                    for jjj=startjjj:imagesize(2)
                        ...
                        if isempty(t0pos) == 1 % guess t0 by peak
                            ...
                            t(:) = (delaypos(:)-t0pos)*4/cmmps; % time vector in ps
                        end
                        ...
                        %         if isscalar(setpulsewidth) % single pulse width
                        %             lb(7) = setpulsewidth*sqrt(2); ub(7) = setpulsewidth*sqrt(2);
                        %         else % multiple pulse widths
                        %             if length(setpulsewidth) == numel(subfolders) % if one per folder
                        ...
                        % Baseline fitting
                        [basecurve,bleachrate,basefit,lbbase,ubbase,sbr,corrsig,corrsigbase,tbase,sigbase] = ...
                            basefitfn(t,signal,basefittype,cutlow,cuthigh,troubleshoot);

                        % Calculate summary values
                        [sigpeak,peaksign,peaksd,integsig,integsd,snr,snrv2,snrsweep,intsnr] = ...
                            summaryvals(corrsig,corrsigbase,sigsds);

                        % Lifetime fitting
                        [tint,sigint,fitvector,tfit,fitcurve,fitval,...
                            lifetime1,lifetime2,lt1lt2,fwhm,r2,resid,srr,fftx,ffty,currltfittype] = ...
                            ltfitfn(t,signal,currltfittype,peaksign,sigpeak,...
                        ...
                        outname = string(currentfile(1:end-4));
                        % if currfiletype > 1 && pairedDCAC > 0
                        %     outname = string(currentfile(1:end-8)) + "_DCAC";
                        % else
                        %     outname = string(currentfile(1:end-4));
                        % end
                        ...
                        if powernormtype > 0 % prepare y-axis normalization label
                            normlabel = 'Norm. ';
                            if powernormtype == 1
                                pmtunitlabel = '(V/mW)';
                        ...
                        if currdatatype == 0 % solution data -> write master array
                            ...
                            if pairedDCAC > 0
                                master(1:length(DC),indcrawCH1,ii,jj) = DC(:);
                                master(1:length(corrDC),indccorrCH1,ii,jj) = corrDC(:);
                                master(1:length(DCbasecurve),indcCH1base,ii,jj) = DCbasecurve(:);
                                master(indrCH1peak,indcvalue,ii,jj) = DCpeak;
                                master(indrCH1peaksd,indcvalue,ii,jj) = DCpeaksd;
                                master(indrintegCH1,indcvalue,ii,jj) = integDC;
                                master(indrCH1integsd,indcvalue,ii,jj) = DCintegsd;
                                master(indrCH1snr,indcvalue,ii,jj) = DCsnr;
                                master(indrCH1sbr,indcvalue,ii,jj) = DCsbr;
                                master(indrCH1noise,indcvalue,ii,jj) = DCnoise;
                                master(indrCH1bleach,indcvalue,ii,jj) = DCbleachrate;
                            end
                            ...
                            if writeprocyn == 1 % write files
                                writematrix(squeeze(master(:,:,ii,jj)),outname+'_proc.dat',...
                                    'FileType','text')
                            end
                        end
                    end
                end
                if writeprocyn == 1 && currfiletype == 3 % write processed image tif
                    Save_tif(outname+'_proc.tif',lt1array,...
                        lt2array,bfarray,fwhmarray,r2array)
                end
                % if emptytype == 1
        ...
        %     currfiletype = []; % empty for next folder
        % end
    end

    if writeprocyn == 1 && currfiletype ~= 3 % write batch file
    ...
    workingdirectory = pwd; % get working directory
    directorycontents = dir(fullfile(workingdirectory,'*')); % search working directory
    subfolders = setdiff({directorycontents([directorycontents.isdir]).name},{'.','..'}); % subfolders of working directory
    emptydatcount = 0;
    for ii = folders
...
% %% Manual fitting -- function should remove need for this
...
for i=1:length(subset)
    ...
    IRpowers(1:nfiles,i) = squeeze(master(indrIRpower,indcvalue,subset(i),1:nfiles));
    prbpowers(1:nfiles,i) = squeeze(master(indrprbpower,indcvalue,subset(i),1:nfiles));
end
...
%% Function testing - baseline fitting

% folders = [1 2 3];
% t0pos = [];
% currt0 = setvalue(t0pos,folders);
[testbase,~,~,~,~,~] = basefitfn(t,signal,basefittype,cutlow,cuthigh,troubleshoot);

figure; plot(t,testbase,'o',t,basecurve,'-');
xlabel('Time (ps)'); ylabel('Signal (AU)');
legend('New function','Old hard code');
...
[delaypos,ch2data,ch2,sigsds,imagesize] = ...
    loaddata(workingdirectory,subfolders,ii,currentfile,...
    currfiletype,trimlastT,trimfirstT,xsteps,ysteps,2);
figure; plot(delaypos,ch2,'o',x,signal,'-');

% Load CH1
ch1file = string(currentfile(1:end-7))+'CH1.raw';
[~,ch1data,ch1,ch1sds,~] = ...
    loaddata(workingdirectory,subfolders,ii,ch1file,...
    currfiletype,trimlastT,trimfirstT,xsteps,ysteps,1);
figure; plot(delaypos,ch1,'o',x,DC,'-');


%% Viewing image
image(data(:,:,1),'CDataMapping','Scaled'); colormap('hot'); colorbar; datacursormode;
...
%%% Output: set value (scalar)
%%% Inputs: intended pre-set(s) (scalar for global, vector for per-folder),
% folders to be analyzed, current folder
...
function [DELAYPOS,DATA,IMAGESIZE] = ...
    loaddata(WORKINGDIRECTORY,SUBFOLDERS,II,CURRENTFILE,CURRFILETYPE,TRIMLASTT,TRIMFIRSTT,XSTEPS,YSTEPS,CHANNEL)
%%% This function loads data from .txt, .raw, or .tif files and
% automatically trims them as specified
%%% Output: delay stage positions, raw data array, signal (Tsweep), 
% signal stdevs, image size
%%% Inputs: working directory, subfolders, current subfolder, current file, 
% current file type, T values to trim (from end and start), xsteps, ysteps,
% and intended data channel
    ...
    if CURRFILETYPE == 1 % load data from .txt
        ...
        if CHANNEL == 1
            SIGNAL = DATA(:,2);
        else % .txts only have ch1 and ch2
            SIGNAL = DATA(:,3);
        end
        % if width(data) >= 3
        %     CH1 = data(:,2);
        ...
        % else
        %     SIGNAL = data(:,2);
        %     CH1 = zeros(height(SIGNAL),width(SIGNAL));
        % end
        ...
        SIGNAL = SIGNAL(TRIMFIRSTT+1:end-TRIMLASTT);
        SIGSDS = zeros(height(SIGNAL),width(SIGNAL)); % no stdev data present - write as zeros

        % No extra channels
        % CH1 = CH1(TRIMFIRSTT+1:end-TRIMLASTT);
        % CH1SDS = SIGSDS;
        % CH3 = SIGSDS; CH3SDS = CH3; CH4 = CH3; CH4SDS = CH3; SPCM = CH3; SPCMSDS = CH3;
        % if pairedDCAC > 0
        %     DCsds = zeros(height(SIGNAL),width(SIGNAL));
        % end
    else % load Tlist and image
        % Look for Tlist
        if isfile(string(CURRENTFILE(1:end-7))+'Tlist.txt') % check for individual Tlist % exist(currentfile(1:end-7)+'Tlist.txt','file') == 2
            DELAYPOS = importdata(string(CURRENTFILE(1:end-7))+'Tlist.txt');
        else
            Tlistname = fullfile(WORKINGDIRECTORY,SUBFOLDERS{II},'Tlist.txt');
            if isfile(Tlistname) % exist('Tlist.txt','file') == 2
                ...
                DELAYPOS = linspace(176,180.05,82);
            end
        end
        ...
        %     Tlistanswer = str2double(Tlistanswer);
        %     numberofpoints = 1+(Tlistanswer(2)-Tlistanswer(1))/Tlistanswer(3);
        %     DELAYPOS = linspace(Tlistanswer(1),Tlistanswer(2),numberofpoints);
        % end
        ...
        if CURRFILETYPE < 4 % (already ~=1) -> .tif
            ...
            DATA = reshape(imgvector,[XSTEPS,YSTEPS,length(DELAYPOS)]);
            % for i=1:length(x) % flip x and y
            %     data(:,:,i) = data(:,:,i).';
            % end
        end
        ...
        CURRFILETYPE
        %%% move to main script or rework?
        % if CURRFILETYPE == 2 || CURRFILETYPE == 4 % avg & stdev of XY data (|| = OR)
        %     SIGNAL = squeeze(mean(DATA, [1 2]));
        %     SIGSDS = squeeze(std(DATA, 0, [1 2]));
        %     % CH1 = squeeze(mean(DCdata, [1 2]));
        %     % DCsds = squeeze(std(DCdata, 0, [1 2]));
        %     IMAGESIZE = size(SIGNAL);
        % else % image data - make arrays to write outputs into
        %     % r2array = zeros([IMAGESIZE(1) IMAGESIZE(2)]);
...
%%% 
    ...
    %%% removed b/c peak power not changed by chopping, only avg (also b/c added AOM back in)
    % if DFGWN == IRWN
    %     IRpower = IRpower/2; % lose 50% power to chopper (DFG only)
    % end
...
% Baseline fitting
function [BASECURVE,BLEACHRATE,BASEFIT,LBBASE,UBBASE,SBR,CORRSIG,CORRSIGBASE,TBASE,SIGBASE] = ...
    basefitfn(T,SIGNAL,CURRBASEFITTYPE,CUTLOW,CUTHIGH,TROUBLESHOOT)
%%% This function performs baseline fitting and can determine which function
% to use based on the variation of the data
%%% Outputs: baseline curve, bleach rate (ps-1), baseline fit vector,
% lower bounds, upper bounds, signal-to-background ratio
%%% Inputs: time vector, signal vector, baseline fitting type, lower cutoff,
% upper cutoff, troubleshooting status

    % Setup baseline fitting
    ilow = ceil(length(T)*CUTLOW); % using ceiling to round up
    ihigh = ceil(length(T)*CUTHIGH);
    if isempty(CURRBASEFITTYPE) == 1
        frontmean = mean(SIGNAL(1:ilow));
        tailmean = mean(SIGNAL(ihigh:end));
        fronttaildiff = 2*(frontmean-tailmean)/(frontmean+tailmean);
        if fronttaildiff > 0.03
...
% Summary values
function [SIGPEAK,PEAKSIGN,PEAKSD,INTEGSIG,INTEGSD,SNR,SNRV2,SNRSWEEP,INTSNR] = summaryvals(CORRSIG,CORRSIGBASE,SIGSDS)
%%% This function calculates summary values for data after baseline
% correction
%%% Inputs: corrected signal, corrected signal baseline
%%% Outputs: peak sign, stdev at peak, integrated signal, integrated
% stdev, snr (range), snr (stdev), snr sweep, integrated snr sweep
...
function [TINT,SIGINT,FITVECTOR,TFIT,FITCURVE,FITVAL,LIFETIME1,LIFETIME2,LT1LT2,FWHM,R2,RESID,SRR,FFTX,FFTY,CURRLTFITTYPE] = ...
    ltfitfn(T,SIGNAL,CURRLTFITTYPE,PEAKSIGN,SIGPEAK,CURRPULSEWIDTH,LTMIN,LTMAX,BASEFIT,CURRBASEFITTYPE,FLOATBASE,IRWNum,TROUBLESHOOT)
%%% This function is performs lifetime fitting (convolution of Gaussian
% with vibrational decay
%%% Inputs: time vector, signal vector, lifetime fitting type, baseline pre-fit vector, 
% baseline fitting type, troubleshooting status
%%% Outputs: interpolated time vector, interpolated signal vector, fit
% value vector, r^2 of fit, residuals of fit
    ...
    if isempty(CURRLTFITTYPE) == 1 % auto-choose lifetime fitting
        if IRWNum > 1680 && IRWNum < 2400 % triple bond or carbonyl
            CURRLTFITTYPE = 1; % single exponential
        else
            % if IRWN > 1293 && IRWN < 1315 && prbWL > 699 && prbWL < 760
            %     CURRLTFITTYPE = 4; % biexp (exp + strexp)
            % else
...
% % Filename parsing (scrapped)
% function [CURRDATATYPE,CURRFILENAMECONV,PRBWL,PRBPOWER,SIGNALWL,IRPOWER,IDLERWL,NDF,PMTG,MODFREQ,PMTBW] = ...
%     parsefilename(CURRENTFILE,CURRDATATYPE,CURRFILENAMECONV)
% %%% This function reads information about the experiment from the name of
% % the file
% %%% Inputs:
% %%% Outputs: current data type (updated), current filename convention
% % (updated), 
% 
%     % Filename parsing
%     macseparator = count(CURRENTFILE,'/');
...
%         end
%     end
%     if setfilenameconv == 1 % parse .txts
%         fileinfo = split(folderinfo(end),"-"); % split filename at "-"s
%         subfolderinfo = split(subfolders{ii}," "); % split subfolder by spaces
%         prbWLstrfull = char(fileinfo(1)); prbWLstr = prbWLstrfull(1:end-1); prbWL = sscanf(prbWLstr,'%f'); % extract probe wavelength and convert to double
%         prbpowerstrfull = char(fileinfo(2)); prbpowerstr = prbpowerstrfull(1:end-2); prbpower = sscanf(prbpowerstr,'%f'); % extract probe power
